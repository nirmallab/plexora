...
#: Versioned, because a cached card is served forever and never re-checked.
#: The unversioned name held what the reader USED to draw -- for an
#: interleaved RGB slide (a Visium HD run's H&E) one row of pixels, 56 bytes --
#: and a fixed reader behind the same name would never have been asked.
#: Bump it whenever what `generate_thumbnail` draws changes.
_THUMBNAIL_CACHE_NAME = '.thumbnail-v2.webp'
#: Earlier names, removed once their replacement is written.
_STALE_THUMBNAIL_NAMES = ('.thumbnail.webp',)
...
def project_thumbnail(name):
    ...
    try:
        ...
        for stale in _STALE_THUMBNAIL_NAMES:
            (cache_path.parent / stale).unlink(missing_ok=True)
    except OSError:
