 ...
 * shown when no datasource is loaded yet). A path is always POSTed -- never
 * the file's bytes, so huge OME-TIFFs load instantly instead of being copied
 * over HTTP.
 *
 * **It POSTs to `/import/sample`, which is the import dialog's own route.**
 * This page is an ENTRY POINT, not an importer: it asks for one path and
 * submits it, and everything that happens after -- what the files are, which
 * of them is the reference, what the sample is called, whether this data is
 * already registered -- is decided by `import_proposal` and `import_sample`
 * exactly as it is for every other surface. That is the whole difference from
 * the `POST /quick_view` this used to call, which was a second importer with
 * its own rules and could open exactly one image.
 *
 * So the page looks the way it always did and can now do more than it could:
 * a Xenium run or a SpatialData store dropped here brings its transcripts,
 * its mask and its table with it, and lands in the viewer with the layer
 * stack already built. Anything still building keeps building behind the
 * viewer, which polls `/import/status` for it (see main.js).
 *
 * What this page deliberately does NOT do is ask questions. A pick that makes
 * several samples, or wants a name or a dataset chosen first, is a decision,
 * and decisions belong in the dialog -- which is what the footer link opens.
 *
...
PlexoraPage.register(function () {
    ...
    /** What `/import/sample` should be given: a path, or a node address. */
    ...
    /**
     * Hand the picked path to the one importer and go where it says.
     *
     * `paths` is a list because that is the route's shape -- the dialog sends
     * everything the user accumulated -- and this page sends exactly one,
     * which is its whole design. No `name` and no `dataset`: both are
     * decisions, and a page whose promise is "no setup" does not get to ask.
     * Detection names the sample, `_dedupe_dataset_name` keeps it unique, and
     * data that is already registered comes back as the project it is already
     * in (`existing`) rather than as a second copy of itself -- the rule this
     * page has always followed, now enforced in one place for every surface.
     *
     * The redirect happens as soon as the RECORD exists. Layers that still
     * have to be built (a mask's pyramid, a transcript index) are built behind
    ...
    // What the import dialog's own `?` opens, from the surface that asks the
    // same question first. No controller of its own: the modal is a classic
    // script on `window`, loaded by base.html everywhere.
    document.getElementById("quick_view_help")?.addEventListener(
        "click", (event) => {
            window.PlexoraImportHelp?.open({returnTo: event.currentTarget});
        });
