/**
 * Does the transcripts plugin's client actually come up?
 *
 ...
 * for this plugin. The controller names TranscriptLayer from inside a method,
 * which runs after toolLoader has awaited every script, so the bindings resolve
 * whatever sequence they arrived in. A file left OUT of the tuple is a real
 * failure, and is caught here.
 *
 ...
 *
 * Run directly:
 *   node tests/js/transcripts_boot_probe.mjs transcriptLayer.js ...
...
if (!problems.length) {
    ...
    } else if (registered[0].name !== "transcripts") {
    ...
    try {
        runInContext(
            "globalThis.__built = (() => {"
            + " const c = new TranscriptsSidebarController({ url: (p) => p, datasource: 'd',"
