"""Baseline smoke checks for a local Plexora datasource.

Defaults to the `orion2` datasource; override with the
PLEXORA_BASELINE_DATASOURCE env var (e.g. `orion_mac` on macOS, since
...
DATASOURCE = os.environ.get("PLEXORA_BASELINE_DATASOURCE", "orion2")
...
def _skip_reason():
    try:
        ...
        return f"Plexora app could not be imported: {exc}"
